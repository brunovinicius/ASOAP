...
public class CacheRepository {
	...
	public static ClassData getClassData(Class<?> clazz) {
		...
		if (classData == null) {
			throw new IllegalArgumentException("Class " + clazz.getSimpleName()
					+ " doesn't have a SOAPObject annotation.");
		}
	...
	public static ServiceData addServiceInterface(Class<?> declaringClass, URL serviceUrl) throws MalformedURLException {
		...
		if (annotation == null) {
			throw new IllegalArgumentException("Class " + declaringClass.getSimpleName()
					+ " doesn't have a SOAPServiceInterface annotation.");
		}
	...
	private static Parser createParserInstance(Class<? extends Parser> parserClass) {
		try {
			...
			throw new IllegalArgumentException("MethodResponseParser " + parserClass
					+ " defines no public, no-args constructor. Could not instantiate.");
		}
	}
		...
		if (type instanceof ParameterizedType) {
			...
			if (!Collection.class.isAssignableFrom(clazz) && !Map.class.isAssignableFrom(clazz))
				throw new IllegalArgumentException(
						"Only Collections are supported for having generic type arguments (i.e: Map<K, V>, List<T>, etc).");
			...
			if (genericTypes.length > 0) {
				...
				if (Map.class.isAssignableFrom(clazz)) {
					...
					throw new UnsupportedOperationException(
							"For Collecrtions, only Map<K, V> and List<T> are currently supported.");
				}
				...
				throw new IllegalArgumentException(
						"Type "
								+ clazz.getSimpleName()
								+ " is a Collection, but it has no declared generic types. Could not determine the appropriate parser for it.");
			}
			...
			if (clazz == Void.class) { // for void types
				...
				throw new IllegalArgumentException(
						"Could not determine a propper parser. Perhaps there's an missing @SOAPObjet annotation for the "
	...
	private static boolean isPrimitive(Class<?> clazz) {
		return clazz == String.class || clazz == boolean.class || clazz == Boolean.class || clazz == char.class
				|| clazz == Character.class || clazz == short.class || clazz == Short.class || clazz == int.class
				|| clazz == Integer.class || clazz == long.class || clazz == Long.class || clazz == float.class
				|| clazz == Float.class || clazz == double.class || clazz == Date.class || clazz == Double.class;
	}
